...
// Queue Node for pokemon BST
typedef struct QueueNode {
   PokemonNode *pokemonNode;
   struct QueueNode *next;
} QueueNode;

// Struct to handle BFS search IO
typedef struct SearchIO {
   int targetID;
   int resultID;
   PokemonNode *result;
} SearchIO;
...
SearchIO *searchIO = NULL;
QueueNode *mergeQueue = NULL;
...
/**
 * @brief Return the starter pokemon ID based on user input
 * @param choice the user input when prompted for a starter choice
 * @return pokedex ID
 * Why we made it: So we can cleanly get starter pokemon ID.
 */
...
/**
 * @brief Create and add an OwnerNode for the circular owners list.
...
/**
 * @brief Combine BFS search + BST removal to remove Pokemon.
 * @param owner owner of pokedex from which to remove
 * @param pokemon the node to remove
 * Why we made it: Self-explanatory
 */
...
/**
 * @brief Search the BST with BFS with SearchIO handling
 * @param root BST root
 * @param visit function pointer
 * Why we made it: BFS.
 */
...
/**
 * @brief Mark a node as 'found' in SearchIO
 * @param node pointer to the node
 * Why we made it: We can pass this to BFSGeneric to find a node in the tree.
 */
void markFoundNode(PokemonNode *node);

/**
 * @brief Mark a nodes succesor as 'found' in SearchIO
 * @param node pointer to the node
 * Why we made it: We can pass this to BFSGeneric to find the successor of a node in the tree.
 */
void markSuccessorNode(PokemonNode *node);

/**
 * @brief Mark a nodes parent as 'found' in SearchIO
 * @param node pointer to the node
 * Why we made it: We can pass this to BFSGeneric to find the parent of a node in the tree.
 */
...
/**
 * @brief Initialize the searchIO
 * Why we made it: SearchIO handles BFS IO
 */
SearchIO *initSearchIO();

/**
 * @brief Clear SearchIO after BFS
 * @param results the SearchIO to clear
 * Why we made it: 'Reset' SearchIO to default state
 */
void *clearSearchIO(SearchIO *results);

/**
 * @brief Init a Queue Node
 * @param node pointer to the pokemon node which is 'stored' in the queue node
 * Why we made it: Queue Nodes handle sorting, mostly
 */
...
/**
 * @brief Remove the first node from the queue
 * @param head pointer to the head of the queue
 * Why we made it: Self-explanatory
 */
...
/**
 * @brief Safely add a node to the queue
 * @param head pointer to the head of the queue
 * Why we made it: enqueue with queue existence checking
 */
 ...
 * @param root BST root
 * Why we made it: Check whether or not a node was added to a BST.
 */
 ...
 * Why we made it: often used in menu choices
 */
